...
export class VerifiedFeishuChannel {
  ...
  async sendImage(chatId, absPath, options = {}) {
    const replyTo = options.replyTo;
    let imageKey;
    try {
      const buffer = await fs.readFile(absPath);
      const res = assertApiSuccess('Feishu image.create', await this.#client.im.v1.image.create({
        data: { image: buffer, image_type: 'message' },
      }));
      imageKey = res?.data?.image_key;
      if (!imageKey) throw new Error('Feishu image.create returned no image_key');
    } catch (error) {
      throw new Error(`Feishu image upload failed: ${error.message}`, { cause: error });
    }
    ...
    assertApiSuccess('Feishu message send (image)', response);
    return response?.data?.message_id ?? null;
  }

  async sendFile(chatId, absPath, options = {}) {
    const replyTo = options.replyTo;
    const fileName = options.fileName ?? path.basename(absPath);
    let fileKey;
    try {
      const buffer = await fs.readFile(absPath);
      const rawRes = await this.#client.im.v1.file.create({
        data: { file: buffer, file_name: fileName, file_type: 'stream' },
      });
      // 调试：记录原始响应
      try {
        const { appendFile } = await import('node:fs/promises');
      } catch {}
      const res = assertApiSuccess('Feishu file.create', rawRes);
      fileKey = res?.data?.file_key ?? res?.file_key;
      if (!fileKey) throw new Error(`Feishu file.create returned no file_key, raw=${JSON.stringify(rawRes).slice(0,500)}`);
    } catch (error) {
      try {
        const { appendFile } = await import('node:fs/promises');
      } catch {}
      throw new Error(`Feishu file upload failed: ${error.message}`, { cause: error });
    }
    ...
    assertApiSuccess('Feishu message send (file)', response);
    return response?.data?.message_id ?? null;
  }
}
