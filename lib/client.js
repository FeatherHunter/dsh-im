...
// package.json
var package_default = {
  name: "@feather_wch/dsh-im",
  version: "0.15.5",
  description: "\u93B6\u5A41\u7BC0\u7EC9?IM \u93C8\u54C4\u6AD2\u6D5C\u54C4\u62F0\u934F\uE102\u7D89 AI Office \u93BA\u30E5\u53C6\u93C8\uE101\u6E80 DeepSeek Harness\u951B\u581F\u656E\u93B8\u4F80\uE5E3\u6D94\uFE3A\u20AC\u4F78\u4E95\u6DC7\xB0\u20AC\u4F80\u62E4\u95BD\u5908\u20AC\u4F77\u7D12\u6D93\u6C2C\u4E95\u6DC7\xB0\u20AC\u4E35Q\u9286\u4E3Clack\u9286\u4E40elegram\u9286\u4E0Fiscord\u9286\u4E44hatsApp \u935C?AI Office\u951B\u5908\u20AC?Connect nine IM channels and a public AI Office to a local DeepSeek Harness. Fork of @xmanrui/dsh-im \u9225?Feishu HTML media fix (private/custom).",
  keywords: [
    "deepseek-harness",
    "dsh",
    "dsh-plugin",
  ...
  author: {
    name: "FeatherHunter",
    url: "https://github.com/FeatherHunter"
  },
  contributors: [
    {
      name: "xmanrui",
  ...
  repository: {
    type: "git",
    url: "git+https://github.com/FeatherHunter/dsh-im.git"
  },
  homepage: "https://github.com/FeatherHunter/dsh-im#readme",
  bugs: "https://github.com/FeatherHunter/dsh-im/issues",
  publishConfig: {
    access: "public"
  },
  bin: {
...
// plugin-src/shared/package-meta.js
var PACKAGE_NAME = package_default.name;
var SCOPE = PACKAGE_NAME.includes("/") ? PACKAGE_NAME.slice(0, PACKAGE_NAME.indexOf("/")) : "";
var UNSCOPED = PACKAGE_NAME.includes("/") ? PACKAGE_NAME.slice(PACKAGE_NAME.indexOf("/") + 1) : PACKAGE_NAME;
var PLUGIN_ID = PACKAGE_NAME.replace(/^@/, "").replace(/\//g, "-").replace(/_/g, "-");
var BASE_ID = PLUGIN_ID;
...
var STANDARD_AGENT_PRESET_ID = "standard";
var LEGACY_AGENT_PRESET_FALLBACKS = Object.freeze({ code: STANDARD_AGENT_PRESET_ID });
...
function normalizeAgentPresetId(value) {
  ...
  const id5 = value.trim().toLowerCase();
  if (Object.hasOwn(LEGACY_AGENT_PRESET_FALLBACKS, id5)) return LEGACY_AGENT_PRESET_FALLBACKS[id5];
...
function WorkspaceDirectoryPicker({
  ...
  const [pathDraft, setPathDraft] = React6.useState(startPath ?? "");
  ...
  const pathInputId = React6.useId();
...
function ContextEnhancementEditor({ config, groupSupported = true, disabled = false, onSave }) {
  ...
  const entryRef = React8.useRef(null);
  const dialogId = React8.useId();
  const statusId = React8.useId();
...
function useWorkspaceSnapshotFence() {
  const state = React9.useRef({ version: 0, pendingMutations: 0 });
  return React9.useMemo(() => Object.freeze({
    beginStatus() {
...
var BotSettingsContext = React10.createContext(Object.freeze({
  openBotSettings() {
  }
}));
...
function BotSettingsButton({
  ...
  const { openBotSettings } = React10.useContext(BotSettingsContext);
  const tooltipId = React10.useId();
  return h2(
    "span",
...
var DINGTALK_STYLE_ID = `${BASE_ID}-dingtalk-settings`;
var CSS = String.raw`
.ddt-page {
  --ddt-accent: #1677ff;
...
var Button = React11.forwardRef(function Button2({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function ConnectionErrorDiagnostic({ error }) {
  ...
  return h2(
    ...
    h2(
      ...
      h2("span", null, "\u9519\u8BEF\u7801"),
      `: ${error.code}`,
      error.referenceId ? h2(React11.Fragment, null, " \xB7 ", h2("span", null, "\u53C2\u8003\u53F7"), `: ${error.referenceId}`) : null
    )
  );
}
...
function DingtalkSettingsTab({ rpcCall }) {
  ...
  const statusRequestRef = React11.useRef(0);
  const workspaceFence = useWorkspaceSnapshotFence();
  const noticeFrameRef = React11.useRef(null);
  const focusFrameRef = React11.useRef(null);
  React11.useEffect(() => {
    mountedRef.current = true;
    return () => {
      mountedRef.current = false;
  ...
  React11.useEffect(() => installDingtalkStyles(), []);
  ...
  const discardStaleFeedback = React11.useCallback((snapshot) => {
  ...
  const focusAddButton = React11.useCallback(() => {
    if (!mountedRef.current) return;
  ...
  const setBotBusy = React11.useCallback((botId, operation) => {
    if (!mountedRef.current) return;
  ...
  const runBotAction = React11.useCallback(async ({ account, operation, endpoint, payload, success }) => {
  ...
  const reconnect = React11.useCallback((account) => runBotAction({
    account,
    operation: "reconnect",
  ...
  const saveWorkspace = React11.useCallback(async (account, workspace) => {
  ...
  const saveBotSetting = React11.useCallback(async (account, operation, endpoint, payload) => {
    const snapshotVersion = workspaceFence.beginMutation();
    setBotBusy(account.botId, operation);
    try {
      const snapshot = normalizeSnapshot(await invoke(
...
var Button3 = React12.forwardRef(function Button4({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function createTokenChannelSettings(definition) {
  ...
  function SettingsTab({ rpcCall }) {
    ...
    const [testNoticeByBot, setTestNoticeByBot] = React12.useState({});
    ...
    const loadStatus = React12.useCallback(async ({ signal, silent = false } = {}) => {
      const workspaceVersion = workspaceFence.beginStatus();
    ...
    const bindCredentials = React12.useCallback(async (values) => {
      const snapshotVersion = workspaceFence.beginMutation();
      setBusy(true);
      setCredentialError(null);
      try {
...
var DISCORD_STYLE_ID = `${BASE_ID}-discord-settings`;
var CSS2 = String.raw`
...
function useAnimationFrameScheduler() {
  const schedulerRef = React13.useRef(null);
  React13.useEffect(() => {
...
var Button5 = React14.forwardRef(function Button6({ children, kind = "secondary", size, icon, className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function QrPane({ provision, now, onRefresh, onCancel, busy }) {
  ...
  React14.useEffect(() => setImageFailed(false), [qrSource]);
  return h2(
    "div",
...
function RemoveConfirmation2({ bot, busy, onConfirm, onCancel }) {
  ...
  React14.useEffect(() => cancelRef.current?.focus(), []);
  return h2(
    "div",
    {
...
function BotCard({
  ...
  const repairTooltipId = React14.useId();
...
function FeishuSettingsTab({ rpcCall }) {
  ...
  const [pageBusy, setPageBusy] = React14.useState(false);
  const [provisionBusy, setProvisionBusy] = React14.useState(false);
  const [credentialOpen, setCredentialOpen] = React14.useState(false);
  const [credentialBusy, setCredentialBusy] = React14.useState(false);
  const [credentialError, setCredentialError] = React14.useState(null);
  const [busyByBot, setBusyByBot] = React14.useState({});
  const [errorsByBot, setErrorsByBot] = React14.useState({});
  const [testNoticesByBot, setTestNoticesByBot] = React14.useState({});
  const [removeTargetId, setRemoveTargetId] = React14.useState(null);
  const [announcement, setAnnouncement] = React14.useState("");
  const [now, setNow] = React14.useState(() => Date.now());
  const [focusBotId, setFocusBotId] = React14.useState(null);
  const cardRefs = React14.useRef(/* @__PURE__ */ new Map());
  const removeButtonRefs = React14.useRef(/* @__PURE__ */ new Map());
  const targetedProvisionRef = React14.useRef(null);
  ...
  const mergeSnapshot = React14.useCallback((snapshot, { restoreProvisioning = false } = {}) => {
    const now2 = Date.now();
  ...
  const loadStatus = React14.useCallback(async ({ signal, silent = false, restoreProvisioning = false } = {}) => {
    const workspaceVersion = workspaceFence.beginStatus();
  ...
  const startProvisioning = React14.useCallback(async ({
    replace = false,
  ...
  const setBotError = React14.useCallback((botId, error) => {
  ...
  const repairCallback = React14.useCallback((connection) => {
  ...
  const reconnectOneBot = React14.useCallback(async (connection) => {
  ...
  const saveWorkspace = React14.useCallback(async (connection, workspace) => {
  ...
  const saveBotSetting = React14.useCallback(async (connection, operation, endpoint, payload) => {
    const { botId } = connection;
    const snapshotVersion = workspaceFence.beginMutation();
  ...
  const requestRemove = React14.useCallback((connection) => {
    setRemoveTargetId(connection.botId);
  }, []);
  const cancelRemove = React14.useCallback(() => {
  ...
  const confirmRemove = React14.useCallback(async (connection) => {
    const { botId, bot } = connection;
    const snapshotVersion = workspaceFence.beginMutation();
  ...
  const setCardRef = React14.useCallback((botId, node) => {
    if (node) cardRefs.current.set(botId, node);
    else cardRefs.current.delete(botId);
  }, []);
  const setRemoveButtonRef = React14.useCallback((botId, node) => {
...
var FEISHU_STYLE_ID = `${BASE_ID}-feishu-settings`;
var CSS3 = String.raw`
.bxf-page {
...
var QQ_STYLE_ID = `${BASE_ID}-qq-settings`;
var CSS4 = String.raw`
...
var Button7 = React15.forwardRef(function Button8({ children, kind = "secondary", className = "", ...props }, ref) {
  return h2("button", {
    ...props,
    ref,
    type: "button",
...
function OfficeSettingsTab({ rpcCall, initialStatus }) {
  const [model, setModel] = React16.useState(normalizeOfficeStatus(initialStatus));
  const [phase, setPhase] = React16.useState(initialStatus === void 0 ? "loading" : "ready");
  const [busy, setBusy] = React16.useState("");
  ...
  const load = React16.useCallback(async () => {
    try {
...
var OFFICE_STYLE_ID = `${BASE_ID}-office-settings`;
var CSS5 = `
...
var SLACK_STYLE_ID = `${BASE_ID}-slack-settings`;
var CSS6 = String.raw`
...
function SlackCredentialPanel({ busy, error, onSubmit, onCancel }) {
  const [botToken, setBotToken] = React17.useState("");
  const [appToken, setAppToken] = React17.useState("");
  const [copied, setCopied] = React17.useState(false);
  const headingId = React17.useId();
...
var TELEGRAM_STYLE_ID = `${BASE_ID}-telegram-settings`;
var CSS7 = String.raw`
...
var WECOM_STYLE_ID = `${BASE_ID}-wecom-settings`;
var CSS8 = String.raw`
...
var WEIXIN_STYLE_ID = `${BASE_ID}-weixin-settings`;
var CSS9 = String.raw`
.dxw-page {
  --dxw-accent: #07c160;
...
var WHATSAPP_STYLE_ID = `${BASE_ID}-whatsapp-settings`;
...
function GroupResponseModeEditor({
  ...
  const [authorizing, setAuthorizing] = React22.useState(false);
...
function FeishuGroupSettingsPage({ account, rpcCall }) {
  const [settings, setSettings] = React22.useState(() => groupSettingsFrom(account));
  const [refreshing, setRefreshing] = React22.useState(false);
  const [refreshError, setRefreshError] = React22.useState(null);
  const [provision, setProvision] = React22.useState(null);
  const [provisionBusy, setProvisionBusy] = React22.useState(false);
  ...
  const applySnapshot = React22.useCallback((value) => {
    const result = targetBotFromSnapshot(value, account.botId);
    if (mounted.current) setSettings(groupSettingsFrom(result.bot));
    return result;
  }, [account.botId]);
  const loadSettings = React22.useCallback(async ({ signal, restoreProvisioning = false } = {}) => {
    setRefreshing(true);
  ...
  const saveSetting = React22.useCallback(async (endpoint, payload) => {
    const value = await invoke(endpoint, { botId: account.botId, ...payload });
    applySnapshot(value);
  }, [account.botId, applySnapshot, invoke]);
  const startAuthorization = React22.useCallback(async ({ replace = false } = {}) => {
  ...
  const finishAuthorization = React22.useCallback(async (signal) => {
  ...
  const cancelAuthorization = React22.useCallback(async () => {
...
function botSettingsTabsForChannel(channel4) {
  return channel4 === "feishu" ? FEISHU_BOT_SETTINGS_TABS : BOT_SETTINGS_TABS;
}
...
function TargetRow({ definition, target, botId, connected, rpcCall, onChanged, onEdit }) {
  ...
  const [testState, setTestState] = React23.useState(null);
  const [syncFeedback, setSyncFeedback] = React23.useState(null);
...
function DeliveryTargetSettingsPage({
  ...
  return h2(
    ...
    h2(
      ...
      }) : activeTab.id === "group" && channel4 === "feishu" ? h2(FeishuGroupSettingsPage, {
        account,
        rpcCall: accessRpcCall
      }) : h2(
        React23.Fragment,
...
function GlobalSettingsPanel({ rpcCall }) {
  ...
  const [ttlInput, setTtlInput] = React24.useState("");
  const [savedTtl, setSavedTtl] = React24.useState(null);
  const [ttlError, setTtlError] = React24.useState(false);
  const [saveError, setSaveError] = React24.useState(null);
  const [saveSucceeded, setSaveSucceeded] = React24.useState(false);
  const [isSaving, setIsSaving] = React24.useState(false);
  const [sweepConfirming, setSweepConfirming] = React24.useState(false);
  const [sweeping, setSweeping] = React24.useState(false);
  const ttlErrorId = React24.useId();
  const ttlHintsId = React24.useId();
  const sweepTriggerId = React24.useId();
  const sweepConfirmId = React24.useId();
  const sweepConfirmTextId = React24.useId();
  const sweepConfirmButtonId = React24.useId();
  const mounted = React24.useRef(true);
  const saving = React24.useRef(false);
  ...
  const loadSettings = React24.useCallback(async ({ signal } = {}) => {
    setPhase("loading");
    setLoadError(null);
    try {
...
var IM_STYLE_ID = `${BASE_ID}-settings`;
...
function manualUpdateCommand(snapshot) {
  ...
  return `dsh plugin --profile ${profileArgument} add -w ${PACKAGE_NAME}@${version}`;
}
function ManualUpdateCommand({ command, disabled, sourceInstall, desktop }) {
  ...
  return h2(
    ...
    command ? h2(
      ...
      command.endsWith(`${PACKAGE_NAME}@latest`) ? h2(
        "p",
...
function IMSettingsTab({
  ...
  const generalSettingsTooltipId = React26.useId();
        ...
        h2(
          ...
          CHANNELS.map((channel4) => h2(
            "button",
            {
              ...
              "aria-selected": !globalSettingsSelected && channel4.id === active.id,
...
function apply(ctx) {
  ...
  const globalSettingsRpcCall = (endpoint, payload, signal) => ctx.connection.rpc.call(GLOBAL_SETTINGS_RPC_CHANNEL, endpoint, payload, signal);
